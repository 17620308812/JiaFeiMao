...
public class GetModelData3_4 {
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (int i = 1; i < 157; i++) {
            ...
            String substring = model.getTextures().get(1).substring(model.getTextures().get(1).lastIndexOf("/") + 1);
            String substring1 = substring.substring(0, substring.lastIndexOf("-upper."));

            String substring2 = model.getTextures().get(3).substring(model.getTextures().get(3).lastIndexOf("/") + 1);
    ...
    public static void model(String basePath, String str, Model model) throws IOException {
        ...
        String sc = model.getTextures().get(1).substring(model.getTextures().get(1).lastIndexOf("/") + 1);
        String sc1 = sc.substring(0, sc.lastIndexOf("-upper."));

        String sc2 = model.getTextures().get(3).substring(model.getTextures().get(3).lastIndexOf("/") + 1);
        String sc3 = sc2.substring(0, sc2.lastIndexOf("."));


        String a = model.getModel().replace("../model", "/static").replace("/model.moc", "/" + sc1 +"-"+sc3+ "/moc/model.moc");
...
//=====================
        ...
        for (String s : textures) {
            ...
            split[split.length - 2] = sc1 +"-"+sc3 + "/textures";
...
//=====================
        ...
        if (expressions != null) {
            ...
            for (ModelExpressions m : expressions) {
                String c = model.getModel().replace("../model", "/static").replace("/model.moc", "/" + sc1 +"-"+sc3 + "/exp/" + m.getName() + ".json");
